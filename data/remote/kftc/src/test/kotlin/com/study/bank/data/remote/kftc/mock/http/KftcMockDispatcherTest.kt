...
class KftcMockDispatcherTest {
    ...
    fun `조회 경로에 POST하면 405와 MethodNotAllowed 메시지`() {
        ...
        assertTrue("메서드와 경로를 메시지에 포함: $body", body.contains("POST") && body.contains("list_finuse"))
    }

    @Test
    fun `이체 경로에 GET하면 405 — 본문 누락 400으로 처리되지 않는다`() {
        val (code, body) = get("/v2.0/transfer/withdraw/fin_num")
