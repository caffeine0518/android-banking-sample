 ...
 *
 * 해피패스([HomeFlowEndToEndTest])와 달리 이 테스트는 **실 DI 그래프의 [KftcMockServer] 싱글톤을
 * 테스트가 직접 잡아(@Inject) 서버를 장애 상태로 전환**한다 — Hilt 계측 테스트가 비로소 값을 하는 지점.
 * KftcMockServer가 @Singleton이라, 주입받은 인스턴스가 곧 AccountRepository가 호출하는 그 서버다.
 *
...
class HomeRefreshErrorEndToEndTest {
    ...
    // 실 그래프가 쓰는 바로 그 mock 서버 싱글톤. 이걸로 응답을 테스트가 제어한다.
    @Inject
