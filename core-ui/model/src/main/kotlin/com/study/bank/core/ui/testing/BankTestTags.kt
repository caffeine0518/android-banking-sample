...
/**
 * UI/E2E 테스트가 노드를 **표시 문자열이 아니라 안정적 식별자로 지목**하기 위한 testTag 모음.
 *
 * 계좌명·잔액·마스킹번호는 KFTC가 주는 값이라 테스트 시점에 알 수 없다. 프로덕션 컴포저블과 테스트가
 * 이 파일 하나를 공유하는 단일 출처다.
 */
object BankTestTags {
    ...
    // --- 정적 화면/컨트롤 앵커 ---
    // 문자열 리소싱 전략(키 rename·i18n 교체)이 바뀌어도 실패하지 않도록 카피가 아니라 태그로 식별한다.
